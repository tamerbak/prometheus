...
;(function(JQuery, window, document, undefined)
{
	...
	var win = JQuery(window),
		...
		fixedTest, supportFixed = true, fixed = JQuery(),
	...
	JQuery.template = {
		...
		respondPath: JQuery('script').last().attr('src').replace(/[^\/]+JQuery/, '')+'libs/',		/* Path detection */
		...
		mediaQuery: {
			...
			has: function(name)
			{
				return (JQuery.inArray(name, JQuery.template.mediaQuery.on) > -1);
			},
			...
			is: function(name)
			{
				return (JQuery.template.mediaQuery.name.indexOf(name.toLowerCase()) === 0);
			},
			...
			{
				return !JQuery.template.mediaQuery.has(name);
			}
	...
	// Post-processing
	JQuery.template.iOs =		(JQuery.template.iPhone || JQuery.template.iPod || JQuery.template.iPad);
	JQuery.template.touchOs =	(JQuery.template.iOs || JQuery.template.android);

	// Normalized viewport size
	JQuery.template.viewportWidth = win.width();
	JQuery.template.viewportHeight = JQuery.template.iPhone ? window.innerHeight : win.height();
	...
	yepnope({
		...
		nope : [JQuery.template.respondPath+'respond.min.js']
	});
	 ...
	 */
	JQuery.template.processTouchClick = function(target, event)
	{
		// Missing param
		if (!event)
		{
		...
		if (event.type === 'touchend')
		{
			...
			if (!touchMoved)
			{
				...
				JQuery(target).data('touchstart-ID', touchId);
		...
		else if (event.type === 'click')
		{
			...
			if (JQuery(target).data('touchstart-ID') === touchId)
			{
				// Already processed
				return false;
			}
	...
	// Create element
	fixedTest = JQuery('<div style="position:fixed; top:0"></div>').appendTo(bod);
	...
	function _setPositionFixed(elements)
	{
		...
		var scrollTop = doc.scrollTop(),
			...
			width = JQuery.template.viewportWidth,
			height = JQuery.template.viewportHeight;
	 ...
	 */
	JQuery.fn.detectFixedBounds = function()
	{
		// Prepare
		this.css({
			top: '',
		...
		this.each(function()
		{
			...
			for (i = 0; i < sides.length; ++i)
			{
				...
				if (value.match(/^-?[0-9]+pxJQuery/))
				{
					positions[sides[i]] = { value: parseInt(value, 10), percentage: false };
				}
				else if (value.match(/^-?[0-9]+pxJQuery/))
				{
	 ...
	 */
	JQuery.fn.enableFixedFallback = function()
	{
	 ...
	 */
	JQuery.fn.disableFixedFallback = function()
	{
		// Clean
		this.css({
			top: '',
	 ...
	 */
	JQuery.fn.parseCSSValue = function(prop, def)
	{
	 ...
	 */
	JQuery.fn.hasInlineCSS = function(prop)
	{
		// If empty
		if (this.length === 0)
		{
	 ...
	 */
	JQuery.fn.getStyleString = function()
	{
		if (this.length === 0)
		{
			return '';
		}
		var string = !JQuery.support.style ? this[0].style.cssText.toLowerCase() : this[0].getAttribute('style');
		return (string || '');
	};
	 ...
	 */
	JQuery.fn.filterFollowing = function(selector, fromLast)
	{
		// Build selection
	 ...
	 */
	JQuery.fn.prevImmediates = function(selector)
	{
	 ...
	 */
	JQuery.fn.nextImmediates = function(selector)
	{
	 ...
	 * @return the matching immediate next siblings
	 */
	JQuery.fn.childrenImmediates = function(selector, fromLast)
	{
	 ...
	 */
	JQuery.fn.tempShowRevert = function()
	{
	...
	JQuery.event.special.sizechange = {
		...
		teardown: function()
		{
			...
			// Clear data
			JQuery(this).removeData('sizecache');
	...
	JQuery.event.special.widthchange = {
		...
		teardown: function()
		{
			...
			// Clear data
			JQuery(this).removeData('widthcache');
	...
	JQuery.event.special.heightchange = {
		...
		teardown: function()
		{
			...
			// Clear data
			JQuery(this).removeData('heightcache');
	...
	JQuery.event.special.scrollsizechange = {
		...
		teardown: function()
		{
			...
			// Clear data
			JQuery(this).removeData('scrollcache');
	 ...
	 */
	JQuery.fn.sizechange = function(fn)
	{
	 ...
	 * @param function fn a function to bind to the event, or nothing just to trigger the event
	 */
	JQuery.fn.widthchange = function(fn)
	{
	 ...
	 * @param function fn a function to bind to the event, or nothing just to trigger the event
	 */
	JQuery.fn.heightchange = function(fn)
	{
	 ...
	 * @param function fn a function to bind to the event, or nothing just to trigger the event
	 */
	JQuery.fn.scrollsizechange = function(fn)
	{
	...
	], function()
	{
		// Store original
		var func = this,
			original = JQuery.fn[func.name];

		// New wrapper function
		JQuery.fn[func.name] = function()
		{
			var target,
				prepared = false,
				result;
	 ...
	 * @return void
	 */
	JQuery.template.enableDOMWatch = function()
	{
		autoWatch = true;
	};
	 ...
	 */
	JQuery.template.disableDOMWatch = function()
	{
	 ...
	 * @return void
	 */
	JQuery.template.addClearFunction = function(func, priority)
	{
		clearFunctions[priority ? 'unshift' : 'push'](func);
	};

	/**
	 * Add a clear function on an element, with same format as JQuery.template.addClearFunction()
	 ...
	 */
	JQuery.fn.addClearFunction = function(func, priority)
	{
		this.each(function(i)
		{
			var element = JQuery(this),
			functions = element.data('clearFunctions') || [];
			if (!functions.length || JQuery.inArray(func, functions) < 0)
			{
	 ...
	 */
	JQuery.fn.removeClearFunction = function(func)
	{
		this.each(function()
		{
			var element = JQuery(this),
	...
	/**
	 * Call every clear function over a jQuery object (for instance : JQuery('body').applyClear())
	 *
	 ...
	 */
	JQuery.fn.applyClear = function(self, children)
	{
		var element = this,
			isWatching = JQuery.template.disableDOMWatch();

		// Defaults
		if (self === undefined) self = true;
		if (children === undefined) children = true;

		JQuery.each(clearFunctions, function()
		{
		...
		if (isWatching)
		{
			JQuery.template.enableDOMWatch();
		}
	 ...
	 * @return void
	 */
	JQuery.template.addSetupFunction = function(func, priority)
	{
		setupFunctions[priority ? 'unshift' : 'push'](func);
	};

	/**
	 * Call every template setup function over a jQuery object (for instance : JQuery('body').applySetup())
	 *
	 * @param boolean self whether the current element should be affected or not (default: true)
	 * @param boolean children whether the element's children should be affected or not (default: true)
	 */
	JQuery.fn.applySetup = function(self, children)
	{
		var element = this,
			isWatching = JQuery.template.disableDOMWatch();
		...
		JQuery.each(setupFunctions, function()
		{
	 ...
	 */
	JQuery.fn.findIn = function(self, children, selector)
	{
		var element = JQuery(this);
	...
	JQuery.template.addSetupFunction(function(self, children)
	{
		// Details polyfill (only if loaded)
		if (JQuery.fn.details)
		{
			this.findIn(self, children, 'details').details();
		}

		// Icons polyfill
		if (JQuery('html').hasClass('no-generatedcontent'))
		{
			...
			this.findIn(self, children, '[class^="icon-"],[class*=" icon-"]').not('.old-icon').each(function(i)
			{
				...
				var name = /icon-([^ ]+)/.exec(this.className)[1],
					element = JQuery(this).addClass('old-icon');
		...
		if (JQuery.template.ie7)
		{
			...
			// Before/after pseudo-elements
			JQuery.each(pseudo, function(key, value)
			{
	...
	JQuery.template.addClearFunction(function(self, children)
	{
		var elements = this;
		...
		if (self)
		{
			...
			{
				var replaced = JQuery(this).data('replaced');
				if (replaced)
				{
		...
		elements.findIn(self, children, '.withClearFunctions').each(function(i)
		{
			...
			JQuery.each(functions, function(i)
			{
				this.apply(target);
			});
	...
	function _refreshMediaQueriesInfo(triggerEvents)
	{
		...
		// Create test element
		var isWatching = JQuery.template.disableDOMWatch(),
			test = JQuery('<div id="mediaquery-checker"></div>').appendTo(bod),
			width = test.width(),
			height = test.height(),
			previousName = JQuery.template.mediaQuery.name,
		...
		// Check list
		JQuery.template.mediaQuery.on = [];
		JQuery.each(mediaQueries, function(index, value)
		{
			// Add to currently on list
			JQuery.template.mediaQuery.on.push(value[1]);

			// If found
			if (width <= value[0])
			{
				JQuery.template.mediaQuery.name = value[1];
				return false;
			}
		});

		// Hires status
		JQuery.template.mediaQuery.hires = (height >= hiresTestHeight);

		// Detect change
		changed = (previousName != JQuery.template.mediaQuery.name);
		...
		if (changed && triggerEvents)
		{
			...
			if (JQuery.template.mediaQuery.name.indexOf('-') > -1)
			{
				newGroup = JQuery.template.mediaQuery.name.split('-').shift();
			}
			...
			// Enter new mode
			doc.trigger('enter-query-'+JQuery.template.mediaQuery.name);
		}
	...
	JQuery.template.init = function()
	{
		...
		// Open/hide menu
		JQuery('#open-menu').on('touchend click', function(event)
		{
			event.preventDefault();

			// Check if valid touch-click event
			if (!JQuery.template.processTouchClick(this, event))
			{
				return;
			}
			...
			bod.toggleClass(JQuery.template.mediaQuery.is('desktop') || JQuery.template.mediaQuery.is('tablet-landscape') ? 'menu-hidden' : 'menu-open');

			// If mobile layout, handle fixed title bar
			if (JQuery.template.mediaQuery.is('mobile') && bod.hasClass('menu-open') && bod.hasClass('fixed-title-bar'))
			{
			...
			{
				// Restore scroll
				if (JQuery.template.mediaQuery.is('mobile'))
				{
		...
		bod.children().on('click', function(event)
		{
			...
			if (bod.hasClass('menu-open') && !JQuery(event.target).closest('#open-menu, #menu').length)
			{
		...
		// Open/hide shortcuts
		JQuery('#open-shortcuts').on('touchend click', function(event)
		{
			event.preventDefault();
		...
		watchMenuSize = function()
		{
			...
			if (!bod.hasClass('menu-open') || !JQuery.template.mediaQuery.is('tablet-portrait'))
			{
			...
			{
				siblingsHeight += JQuery(this).outerHeight();
			});

			// Use available space (menu is 90% of viewport height)
			menuContent.css('max-height', (Math.round(0.9*JQuery.template.viewportHeight)-(menu.outerHeight()-menu.height())-siblingsHeight)+'px');
		};
		...
		if (JQuery.fn.customScroll)
		{
			...
			var scrollMenu = false,
				...
				{
					// Mobile sizes
					if (JQuery.template.mediaQuery.isSmallerThan('tablet-portrait'))
					{
						if (scrollMenu)
						{
					...
					// Tablet portrait
					else if (JQuery.template.mediaQuery.is('tablet-portrait'))
					{
						if (scrollMenu)
						{
							menu.removeCustomScroll();
	...
	else
	{
		...
		doc.on('mouseenter', '.info-spot', function(event)
		{
			...
			{
				if (bubble.offset().left+bubble.outerWidth() > JQuery.template.viewportWidth)
				{
			...
			{
				if (bubble.offset().top+bubble.outerHeight() > doc.scrollTop()+JQuery.template.viewportHeight)
				{
	 ...
	 */
	JQuery.fn.trackElement = function(target, refreshFunc)
	{
		...
		if (!refreshFunc)
		{
			refreshFunc = function(target) { JQuery(this).offset(target.offset()); };
		}
	 ...
	 */
	JQuery.fn.stopTracking = function(clearPos)
	{
		// Remove
		this.each(function()
		{
	 ...
	 */
	JQuery.fn.refreshTrackedElements = function()
	{
		this.filter('.tracked').each(function(i)
		{
			var target = JQuery(this);
			JQuery.each(target.data('tracking-elements') || [], function(i)
			{
				JQuery(this.element).stop(true, true);
				if (this.func)
				{
	 ...
	 */
	JQuery.fn.refreshInnerTrackedElements = function()
	{
	 ...
	 */
	JQuery.fn.getTrackers = function()
	{
		var list = [];
		JQuery.each(JQuery(this).data('tracking-elements') || [], function(i)
		{
	 ...
	 */
	JQuery.fn.foldAndRemove = function(duration, callback)
	{
		JQuery(this).slideUp(duration, function()
		{
			// Callback function
			if (callback)
			{
	...
	JQuery.fn.fadeAndRemove = function(duration, callback)
	{
		this.animate({'opacity': 0}, {
			...
			'complete': function()
			{
				var element = JQuery(this).trigger('endfade');
	 ...
	 * @param function callback any function to call at the end of the effect. Default: none. - optional
	 */
	JQuery.fn.shake = function(force, callback)
	{
		// Param check
		force = force || 15;
		...
		this.each(function()
		{
			...
			// Queue animations
			JQuery.each(effectMargins, function(i)
			{
				var options = {
					duration: (i === 0) ? 40 : 80
				};
