...
$(document).ready(function()
	...
	forms.each(function(i)
		...
		if(this.id != "sigform"){
			formHref = this.id;	
			form.data('button', $('<a href="#'+formHref+'" class="button anthracite-gradient'+color+(active ? ' active' : '')+'">'+this.title+'</a>')
					.appendTo(formSwitch)
					.data('form', form));
		}
		else{
			formHref = "protogen-wizard.xhtml";
			form.data('button', $('<a href="'+formHref+'" class="button anthracite-gradient'+color+(active ? ' active' : '')+'">'+this.title+'</a>')
					.appendTo(formSwitch)
					.data('form', form));
		}
	...
	blocWidth = formBlock.width()-currentForm.data('width');
	...
	function handleLoginResize()
	{
		...
		if (!centered)
		{
			...
			formWrapper.css('min-width', (blocWidth+maxWidth+20)+'px');
			container.css('margin-top', '');
		}
		else
		{
			formWrapper.css('min-height', '');
			formWrapper.css('min-width', '');
	...
	function centerForm(form, animate, ignore)
	{
		...
		if (centered)
		{
			...
			finalSize1 = blocWidth+form.data('width');
			...
			siblings.each(function(i)
				...
				finalSize1 += $(this).outerWidth(true);
					});

			// Setup
			container[animate ? 'animate' : 'css']({ marginTop: -Math.round(finalSize/2)+'px' });
			container[animate ? 'animate' : 'css']({ marginRight: -Math.round(finalSize1/2)+'px' });
		}
	};
